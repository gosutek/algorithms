...
int chocolate(const int &N, const int &K, vector<int> &p) {
   ...
   * Let j = m : 0 <= m <= j. To sell the chocolate at day m we must've bought
   * it on a previous day [0...m - 1] say day b : 0 <= b <= j - 1. We are trying
   * to find the b that maximizes p(m) - p(b) + c(i - 1, b - 1). For every such
   * day m we must check all N remaining days to maximize the profit for all K
   * transactions. This leads to O(kn^2). However p(m) is constant for the
   * current i iteration so we are really trying to find the b that maximizes
   * c(i - 1, b - 1) - p(b). So for every loop we can keep the maximum c(i - 1,
   * b - 1) - p(b) we have found. So for every j iteration we just have to add
   * p(m) to the previously found maximum c(i - 1, b - 1) - p(b). Therefore for
   * every i iteration we have to check N days once. Therefore the time
   * complexity is O(nk).
   */
